import type { AppManifest, AppState, AppEventListener, WindowPosition, WindowSize, AppMessage } from "./types"

// 기본 창 위치 및 크기
const DEFAULT_POSITION: WindowPosition = { x: 50, y: 50 }
...
export class BrowserOSAppManager {
  ...
  private eventListeners: Map<string, Set<AppEventListener>> = new Map()
  ...
  closeApp(appId: string): void {
    ...
    if (state.isActive) {
      const openApps = Array.from(this.appStates.entries()).filter(
        ([id, s]) => id !== appId && s.isOpen && !s.isMinimized,
      )
